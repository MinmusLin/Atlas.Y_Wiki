<template>
  <TextContent title='DESIGN'
               :title-list="['Overview', 'Database', 'Structure Prediction', 'Evaluation Metrics',
               'Subcellular Localization Prediction', 'Protein Directed Evolution Model']">
    <h1 id='Overview'>Overview</h1>
    <p>BAtlas.Y consists of a main functional pathway and several auxiliary features. The core function of the software
      is the <span class='bold'>design of localization proteins</span>, utilizing the <span class='bold'>signal</span>
      database and <span class='bold'>linker database</span>, along with <span class='bold'>structural prediction
    ...
    <ul>
      <li><span class='bold'>Signal Peptide Database</span></li>
      <p>Our Signal Peptide Dataset is designed to facilitate research on protein subcellular localization and
        transport. Signal peptides are short sequences located at the N-terminus of newly synthesized proteins,
        responsible for determining the subcellular localization of these proteins. The data in this database is sourced
        from the dataset used in the development of the <span class='bold'>DeepLoc2.1</span> deep learning model by
        Marius Thrane Ødum et al. To align with the goals of our project, we specifically filtered the dataset to
        include only proteins from eukaryotic organisms, extracted the signal peptides, categorized them, and assigned
        unique identifiers to enable efficient querying. This dataset is particularly suitable for bioinformatics
        research, protein design, and cell biology experiments focused on predicting subcellular localization.</p>
      <img src='https://static.igem.wiki/teams/5503/accompanying-images/design1-1.webp' alt='Design1'>
      <h5>Figure 1.</h5>

      <li><span class='bold'>Linker Database</span></li>
      <p>We aim to construct a linker database designed to connect our target proteins with signal peptide sequences.
        The dataset is divided into two tables: the Classical Linker Data Table and the Natural Linker Data Table.</p>
      <ol>
        <li><span class='bold'>Classical Linker Table</span>: This section contains linkers that have been extensively
          reviewed in the literature and categorized based on their rigidity and flexibility. The linker sequences in
          this table are suitable for use in protein design, molecular biology, and synthetic biology engineering
          projects.
        </li>
        ...
        <li><span class='bold'>Natural Linker Table</span>: This section consists of short peptides derived from natural
          ...
          peptides and conserved regions, following the method used by the 2021 Sun Yat-sen University iGEM team. We
          used protein sequences from the DeepLoc 2.1 dataset, and identified conserved regions using NCBI’s Conserved
          Domain Database (CDD) and its Batch CD-Search tool.
        </li>
      ...
      <p>Building on this hypothesis, we employed the ESMFold model to predict the three-dimensional structures of all
        ...
        rigidity and flexibility, we conducted a t-test on the classical linkers obtained from the literature. The
        results demonstrated a significant difference between the average B-factors and the rigidity classifications,
        yielding a p-value of <span class='bold'>6.145×10<sup>-14</sup></span>, which confirms that the B-factor can
        indeed serve as a reliable indicator of linker rigidity and flexibility.</p>
      <p>Based on this finding, we subsequently classified the natural linkers extracted from proteins according to
        their rigidity and flexibility, thereby addressing the varying experimental requirements for linker
        flexibility.</p>
      <p>In order to explore the properties of linkers, we also calculated their <span
        class='bold'>hydrophobicity</span> to assess their <span class='bold'>suitability</span> in different biological
        environments. This process utilized the ProteinAnalysis class from the <span class='bold'>Bio.SeqUtils.ProtParam
        </span> module. By analyzing the linker sequences, we can obtain hydrophilic and hydrophobic indices, which will
        provide additional reference points for selecting appropriate linkers in experimental designs.</p>

      <li><span class='bold'>Photocontrolled Protein Interaction Localization Database(PPID)</span></li>
      <p><span class='bold'>Optogenetics</span> is an emerging biotechnological approach that utilizes light-sensitive
        proteins and specific wavelengths of light to precisely regulate the functions and interactions of proteins
        within cells. This method enables researchers to achieve high spatial and temporal resolution control in live
        cells, facilitating a deeper understanding of cellular behaviors and physiological processes.</p>
      ...
      <p>The information regarding the blue light-induced optogenetic pair <span class='bold'>CIBN-CRY2</span> system is
        ...
        interactions.</p>
      <p>For the red light-induced <span class='bold'>PhyB-PIF3</span> system, we referenced the study by Konrad Müller
        et al., titled "A red/far-red light-responsive bi-stable toggle switch to control gene expression in mammalian
        cells" This system demonstrates good reversibility, enabling protein binding under red light induction and
        separation under far-red light induction, thus allowing reversible gene regulation.</p>
      <p>Additionally, concerning the near-infrared light-controlled optogenetic pair, we referenced the research by
        Kaberniuk et al., titled "A bacterial phytochrome-based optogenetic system controllable with near-infrared
        light". The proposed single-domain <span class='bold'>BphP1</span> binding partner <span
          class='bold'>Q-PAS1</span> effectively overcomes the limitations posed by the large size, multi-domain
        structure, and oligomerization behavior of the traditional BphP1-PpsR2 system.</p>
      ...
      <ul>
        <li>Red fluorescent protein <span class='bold'>mCherry</span> (UFQ89828.1) and green fluorescent protein <span
          class='bold'>eGFP</span> (UYB79189.1)
        </li>
        <li>Blue fluorescent protein <span class='bold'>mTagBFP</span> (UFQ89829.1) and orange fluorescent protein <span
          class='bold'>mKO</span> (ADE48842.1)
        </li>
        <li>Purple fluorescent protein <span class='bold'>mKate</span> (WKV34671.1) and yellow fluorescent protein <span
          class='bold'>mVenus</span> (AWK40018.1)
        </li>
      </ul>
      <p>The PPID Database is built upon existing signal peptide and linker databases. For each specific subcellular
        ...
        optogenetic proteins and fluorescent proteins. Subsequently, we utilize the built-in stability calculation
        methods in <span class='bold'>PyRosetta</span> to evaluate the stability of the resulting fusion proteins,
        ultimately storing the combinations with the <span class='bold'>highest stability</span> in our database.</p>
      <p>Through this systematic process, the PPID Database not only provides researchers with a powerful tool to
        explore the applications of optogenetics in cell biology, but also offers a wealth of data resources and
        experimental foundations for future research.</p>
    </ul>
    ...
    <p>In our project’s design process, protein structure prediction is crucial, directly influencing the evaluation of
      ...
      compared several prediction tools and, after considering accuracy, speed, and resource demands, chose <span
        class='bold'>ESMFold.</span></p>
    ...
    <ul>
      <li><span class='bold'>Stability Evaluation</span></li>
      <p>In the Atlas.Y platform, we design fusion proteins that can be precisely localized to specific subcellular
        sites for our users. To achieve this goal, we need to add signal peptides and linkers to the proteins provided
        by users, thereby constructing protein assemblies. However, the introduction of any additional sequences may
        impact the functionality of the original proteins. Therefore, we developed the CAD (<span class='bold'>Contact
        Area Difference</span>) model to evaluate the <span class='bold'>functional changes</span> in fusion proteins.</p>
      <p>The design of the CAD model is based on a deep understanding of the relationship between protein structure and
        ...
        their original counterparts. To accomplish this, we utilize <span class='bold'>Voronoi</span> diagrams and
        convex hull algorithms. We first generate a Voronoi diagram that assigns each atom in the protein structure to
        its respective spatial region, representing the influence area of that atom. This approach allows us to gain a
        clearer understanding of the role of each atom within the structure.Next, we employ the convex hull algorithm to
        compute the contact area between residues. This algorithm accurately reflects the contact regions between
        residues by determining the smallest convex polygon that encompasses the Voronoi regions.</p>
      <p>To comprehensively assess the functional changes in proteins, the CAD model not only calculates overall area
        differences but also allows for localized evaluations, focusing on <span class='bold'>specific functional regions
        </span>. This design enables researchers to gain deeper insights into the changes in interactions near active
        sites or functional domains, aiding in the assessment of how these changes impact protein functionality.</p>
      ...
      <li><span class='bold'>Functional Evaluation</span></li>
      <p>When designing fusion proteins that can be precisely localized to specific subcellular sites for our users, we
        ...
        proteins exhibit good <span class='bold'>stability</span> is crucial, as it directly impacts the <span
        class='bold'>functionality</span> and <span class='bold'>structural</span> integrity of the proteins. To
      ...
      <p>During the stability evaluation process, <span class='bold'>PyRosetta</span> scoring functions take into
        account multiple factors, including van der Waals forces, electrostatic interactions, and hydrogen bonds, to
        calculate the total energy of the fusion proteins. A lower energy score indicates greater structural stability,
        reflecting the potential of fusion proteins to maintain structural integrity, functional efficiency, and
        prolonged lifespan. By integrating PyRosetta into our model, we provide users with a reliable stability
        assessment, ensuring that the designed fusion proteins not only localize accurately but also maintain their
        structural stability, thus facilitating more precise applications in bioengineering.</p>
    </ul>
    ...
    <p>Considering the complex protein sorting mechanisms within yeast, relying solely on signal sequence connections
      ...
      signal sequences but also on their <span class='bold'>folding states</span> and <span class='bold'>microenvironments</span>.
      Different organelles possess unique membrane characteristics and transport mechanisms, necessitating precise
      selection and adjustment of proteins before they reach their destinations.</p>
    <p>However, our current algorithms <span class='bold'>do not</span> take spatial structural features into account,
      ...
      This sensitivity to spatial structure can effectively improve the selectivity and transport efficiency of proteins.</p>
    ...
    <p>This comprehensive approach will assist us in designing targeting strategies with higher accuracy, advancing the
      ...
      protein localization across different organelles, providing strong support for related research and
      applications.</p>
