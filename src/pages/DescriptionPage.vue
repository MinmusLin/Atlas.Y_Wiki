...
const r = 30 // 圆弧半径为30
const start_height = 300 // 起始高度为300
const speed = 10
  ...
  if (typeID == 1) {
    if (current_line >= y) {
      if (current_line > y + r) {
        ...
        if (canDraw(y - 150, y + 150)) {
          ctx.arc(x, y, r, Math.PI, Math.PI - Math.asin((Math.min(current_line, y + r) - y) / r), true)
        }
        if (canvas_h > current_line) {
          current_line += speed
          setTimeout(() => draw(), 5) // 使用箭头函数
    ...
    if (current_line >= y - r && lines[lineID] == x) {
      if (current_line > y) {
        ...
        if (canDraw(y - 150, y + 150)) {
          ctx.arc(x, y, r, Math.PI * 1.5, Math.PI * 1.5 + Math.acos((y - current_line) / r), false)
        }
        if (canvas_h > current_line) {
          current_line += speed
          setTimeout(() => draw(), 5) // 使用箭头函数
    ...
    if (current_line >= y - r && lines[lineID] == x) {
      if (current_line > y) {
        ...
        if (canDraw(y - 150, y + 150)) {
          ctx.arc(x, y, r, Math.PI * 1.5, Math.PI + Math.asin((y - current_line) / r), true)
        }
        if (canvas_h > current_line) {
          current_line += speed
          setTimeout(() => draw(), 5) // 使用箭头函数
